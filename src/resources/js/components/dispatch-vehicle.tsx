...
const DispatchVehicle = ({ onCarrierSelectionChange }) => {
    ...
    const allCarriers = useMemo(
        () =>
            Array.from({ length: 14 }, (_, index) => ({
                ...
                stats: [Math.floor(Math.random() * 5), Math.floor(Math.random() * 5), Math.floor(Math.random() * 5), Math.floor(Math.random() * 5)],
            })),
        [],
    );

    const carriers = useMemo(() => {
        if (!searchTerm.trim()) return allCarriers;
    ...
    const handleSelectAll = useCallback(
        (checked) => {
            ...
            const newSelection = checked ? new Set(carriers.map((carrier) => carrier.id)) : new Set();
        ...
        [carriers, onCarrierSelectionChange],
    );
    ...
    const handleCarrierSelect = useCallback(
        (carrierId, checked) => {
            ...
            if (checked) {
                newSelectedCarriers.add(carrierId);
            } else {
                newSelectedCarriers.delete(carrierId);
            }
            ...
            if (newSelectedCarriers.size === carriers.length && carriers.length > 0) {
                setSelectAll(true);
            } else {
                setSelectAll(false);
            }
        },
        [selectedCarriers, carriers.length, onCarrierSelectionChange],
    );
    ...
    return (
        <div className="w-full lg:w-2/12">
            ...
            <div
                className={`overflow-hidden transition-all duration-300 ease-in-out lg:hidden ${isMobileMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'} `}
            >
...
// Move SidebarContent outside as a separate component
                        ...
                        {carriers.length > 0 ? (
                            carriers.map((carrier) => (
                                <tr key={carrier.id}>
                                    ...
                                    <td className="border px-2">
                                        <p>
                                            {carrier.id} {carrier.status.h && <span className="rounded bg-green-400 px-1">H</span>}
