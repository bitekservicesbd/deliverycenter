...
import CreateUser from '@/pages/centrals/tenant-users/create';
import EditUser from '@/pages/centrals/tenant-users/edit';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Tenant Users', href: route('central.tenant.users.index') },
];
...
export default function Index() {
    ...
    const filteredAndSortedUsers = useMemo(() => {
        let filtered = users.filter((user: User) => {
            const matchesSearch =
                ...
                user.company_name?.toLowerCase().includes(filterText.toLowerCase()) ||
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('central.tenant.users.delete', selectedUserId), {
            onSuccess: () => {
    ...
    const confirmBulkDelete = () => {
        ...
        selectedRows.forEach((planId) => {
            router.delete(route('central.tenant.users.delete', planId), {
                onSuccess: () => {
                    router.reload({ only: ['users'] });
                },
            });
    ...
    const handleExport = () => {
        const csvContent = [
            ['Username', 'Name', 'Company Name', 'Email', 'Status'].join(','),
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="min-h-screen space-y-6 bg-slate-50 p-6 dark:bg-black">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Tenant Users</h1>
                        <p className="mt-1 text-slate-600 dark:text-white">Manage your tenant users</p>
                    </div>
                    <Button
                                        ...
                                        <tr key={user.id} className="transition-colors dark:text-white">
                                            <td className="px-6 py-4 dark:text-white">
                                                <Checkbox
                                                    ...
                                                    className="border-slate-300 dark:text-slate-700"
                                                />
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="text-sm font-bold text-slate-900 dark:text-white">{user.company_name || 'N/A'}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-48">
                                                        ...
                                                        <DropdownMenuItem className="flex cursor-pointer items-center px-2 py-2 text-sm">
                                                            <a
                                                                href={route('login.as.user.panel', user.email)}
            ...
            <DeleteConfirmationModal
                ...
                message={`Are you sure you want to delete ${selectedRows.length} user(s)? This action cannot be undone.`}
